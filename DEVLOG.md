...
## 2026-08-12 — control record 0025

- Theory-of-Constraints focus: identified the fixed-pattern, byte-sensitive SQL normalization and dispatch boundary as the current system constraint. Ordinary equivalent spellings with repeated spaces, tabs, newlines, outer whitespace, or a space before `VALUES (` were rejected before reaching otherwise-supported database behavior.
- Raw-binary intervention: appended opaque function index 54 with the existing `(i32, i32) -> i32` type and inserted an eight-byte call seam in execute function index 1. The helper compacts the declared SQL input window in place before the existing dispatcher, without changing the snapshot format or exported ABI.
- Candidate checkpoint: 19,819 bytes; SHA-256 `126889a0bb110c29e601d450c4e1a4903e435ce1173918114c90f2348b28e1e7`; parent sequence 20 SHA-256 `ca9b5b1bb302243c770af4c25306cbe689d4c532889ecd2a46e66fe0f053d5a8`.
- Early failure and learning: the first membership replay removed the required space in `IN (`. The raw branch intended to compare the preceding byte with ASCII `N`, but encoded positive 78 as the one-byte signed LEB128 byte `0x4e`, which denotes `-50`; the correct signed encoding is `0xce 0x00`. Correcting that byte-level representation restored focused membership behavior.
...
## 2026-08-12 — control record 0031

- Exploration-surface intervention: added an **Explore binary floorplan** control that opens the same active-artifact visualization as a full-screen modal view rather than replacing it with a second model.
